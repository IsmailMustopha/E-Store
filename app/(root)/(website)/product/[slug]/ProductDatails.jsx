"use client";
import { WEBSITE_CART, WEBSITE_PRODUCT_DETAILS, WEBSITE_SHOP } from "@/app/routes/WebsitePanelRoute";
...
const ProductDetails = ({ product, variant, colors, sizes, reviewCount }) => {
  ...
  const currentColor = searchParams.get("color") || variant.color;
  const currentSize = searchParams.get("size") || variant.size;
        ...
        <div className="md:w-1/2 md:mt-0 mt-5">
          ...
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xl font-semibold">
              {variant.sellingPrice.toLocaleString("en-NG", {
                style: "currency",
                currency: "NGN",
              })}
            </span>
            <span className="text-sm line-through text-gray-500">
              {variant.mrp.toLocaleString("en-NG", {
                style: "currency",
                currency: "NGN",
              })}
            </span>
            <span className="bg-red-500 rounded-2xl px-3 py-1 text-white text-sm ms-5">
            ...
            <div className="flex gap-5">
              ...
              {colors.map((color) => (
                <Link
                  key={color}
                  onClick={() => setIsProductLoading(true)}
                  href={`${WEBSITE_PRODUCT_DETAILS(product.slug)}?color=${color}&size=${currentSize}`}
                  className={`border py-1 px-3 rounded-lg cursor-pointer hover:bg-primary hover:text-white ${
                    color === currentColor ? "bg-primary text-white" : ""
                  }`}
                >
                  {color}
                </Link>
            ...
            <div className="flex gap-5">
              ...
              {sizes.map((size) => (
                <Link
                  key={size}
                  onClick={() => setIsProductLoading(true)}
                  href={`${WEBSITE_PRODUCT_DETAILS(product.slug)}?color=${currentColor}&size=${size}`}
                  className={`border py-1 px-3 rounded-lg cursor-pointer hover:bg-primary hover:text-white ${
